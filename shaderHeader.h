...
//create the shaders to be used for your program
void initShaders(ShaderInfo* shaders){
  
  ShaderInfo* shade=shaders;
  vector<GLuint> Shadelist;//initialize a list of shaders
  
  while(shade->type != GL_NONE){//loop through all the shaders in the list then add shaders to new list
    Shadelist.push_back(createShader(shade->type,inputShader(shade->filename)));
    ++shade;
  }
  
  GLuint program=createProgram(Shadelist);//creates a program and links the program to all shaders
  
  glUseProgram(program);//installs current program object as rendering state
}

//loads the shader from the vertex, fragments shaders 
const GLchar* inputShader(const char* filename){
  ...
  //allocates memory for the file then reads in the file 
  ...
  shadingSource[filesize] = 0;//sets a '\0' at end of file
...
GLuint createShader(GLenum type, const GLchar* shadeSource){
  ...
  glCompileShader(shader);//compiles shader object
  ...
  if(!compileStatus){//checks to see if the shader compiled
    ...
    switch(type){//gets what shader has the error
...
GLuint createProgram(const vector<GLuint> shadeList){
  ...
  glBindAttribLocation(program, 1, "in_color");//binds the location an attribute to  program
  glLinkProgram(program);//links the program to your program
